...
public class BasicApplet extends JApplet implements ActionListener,Runnable{
	...
	BasicGame game;
	BasicWorld trainWorld, playWorld; // seperate world from playing world
	RLController rlc;
	RLearner rl;
	...
	JTextField alpha, gamma, epsilon, epochs, reward;
...
/*		Image catImg = getImage(ClassLoader.getSystemResource("cat.gif"));
	...
	private void gameInit(int xdim, int ydim) {
		...
		playWorld = new BasicWorld(xdim, ydim,trainWorld.walls);
		...
		game = new BasicGame(this, DELAY, playWorld, rl.getPolicy());
		game.start();

		// set text fields on panels
		reward.setText(Integer.toString(trainWorld.agentScore));
	...
	public void updateBoard() {
		// update score panels
		agentscorelabel.setText(MS_TEXT+" "+Integer.toString(agentscore));
	...
	void updateScore() {
		double newScore = (double)agentscore;
	...
	Container scorePanel() {
		...
		agentscorelabel = new JLabel(MS_TEXT, mouse, JLabel.RIGHT);
...
class chartPanel extends JPanel {
	...
	public chartPanel(BasicApplet a) {
		this.a = a;
		history = new Vector();
	}
