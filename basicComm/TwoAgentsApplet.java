...
public class TwoAgentsApplet extends JApplet implements ActionListener,Runnable{
	...
	JTextField alpha, gamma, epsilon, epochs, reward1;
	...
	boardPanel bp;
	public int agentsScore=0,catScore=0;
	JLabel agentsScorelabel;

	final String MS_TEXT = "agent Score:";
	JSlider speed, smoothSlider;
	Image  mouseImg,catImg;
	chartPanel graphPanel;
	JLabel winPerc1;

			
	boardObject mouse, cheese,cat, back, hole, wall;
...
/*		Image catImg = getImage(ClassLoader.getSystemResource("cat.gif"));
	...
	public void updateBoard() {
		// update score panels
		agentsScorelabel.setText(MS_TEXT+" "+Integer.toString(agentsScore));
		...
		if(game.getScoringArea()==0){
			bp.setSquare(cheese, new Dimension((int)(BX-3),(int)(BY/2)));
			bp.setSquare(cat, new Dimension((int)(BX-1),(int)(BY/2)));
		}
		else{
			bp.setSquare(cat, new Dimension((int)(BX-3),(int)(BY/2)));
			bp.setSquare(cheese, new Dimension((int)(BX-1),(int)(BY/2)));
...
//		trainWorld.agent2Score = cval2;
	...
	void updateScore() {
		double newScore = (double)agentsScore;
		double catS=(double)catScore;
		winPerc1.setText(String.format("%.5g%n", (newScore/(newScore+catS)))+"%");
	...
	Container scorePanel() {
		...
		agentsScorelabel = new JLabel(MS_TEXT, mouse, JLabel.RIGHT);
		...
		scorePane.add(agentsScorelabel);
