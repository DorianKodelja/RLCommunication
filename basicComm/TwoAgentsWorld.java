...
public class TwoAgentsWorld implements RLWorld {
	...
	public int position; // information about scoring position 0:left/1:rigt/2:top/3:bottom
	...
	static final int NUM_OBJECTS1 = 5, NUM_OBJECTS2 = 4, NUM_ACTIONS1 = 4,NUM_ACTIONS2=4, WALL_TRIALS = 100;
	...
	private boolean agentInCatArea() {
		if (position == 0 && nx == bx-1){
			return true;}
		else if (position == 1 && nx == bx-3){
	...
	private boolean agentInScoreArea() {
		if (position == 0 && nx == bx-3){
			return true;}
		else if (position == 1 && nx == bx-1){
			return true;}
		else
			return false;
	}
