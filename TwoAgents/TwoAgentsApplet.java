...
import Basic.BasicGame;
import Basic.BasicWorld;
...
public class TwoAgentsApplet extends JApplet implements ActionListener,Runnable{
	static final int BW=300, BH=300, BX=8, BY=8, NUM_WALLS=0,
	...
	TwoAgentsGame game;
	TwoAgentsWorld trainWorld, playWorld; // seperate world from playing world
	RLController rlc;
	RLearner rl;
	...
	JTextField alpha, gamma, epsilon, epochs, reward1,reward2;
	...
	boardPanel bp;
	public int agent1score=0,agent2score=0;
	JLabel agent1scorelabel;
	JLabel agent2scorelabel;
	final String MS_TEXT = "agent Score:";
...
/*		Image catImg = getImage(ClassLoader.getSystemResource("cat.gif"));
	...
	public void worldInit(int xdim, int ydim, int numwalls) { 
		trainWorld = new TwoAgentsWorld(xdim, ydim,numwalls);
		gameInit(xdim,ydim);
	}
	public void worldInit(boolean[][] givenWalls) {
		int xdim = givenWalls.length, ydim = givenWalls[0].length;
		trainWorld = new TwoAgentsWorld(xdim, ydim,givenWalls);
		gameInit(xdim,ydim);		
	}
	private void gameInit(int xdim, int ydim) {
		...
		playWorld = new TwoAgentsWorld(xdim, ydim,trainWorld.walls);
		...
		game = new TwoAgentsGame(this, DELAY, playWorld, rl.getPolicy1(),rl.getPolicy2());
		game.start();

		// set text fields on panels
		reward1.setText(Integer.toString(trainWorld.agent1Score));
		reward2.setText(Integer.toString(trainWorld.agent2Score));
	...
	public void updateBoard() {
		// update score panels
		agent1scorelabel.setText(MS_TEXT+" "+Integer.toString(agent1score));
		agent2scorelabel.setText(MS_TEXT+" "+Integer.toString(agent2score));
		...
		bp.setSquare(mouse, game.getAgent());
		bp.setSquare(mouse, game.getAgent2());
		if(game.getScoringArea()==0){
			bp.setSquare(cheese, new Dimension((int)((BX-1)/2),0));
			bp.setSquare(cheese, new Dimension((int)((BX-1)/2),1));
			bp.setSquare(cheese, new Dimension((int)(1+(BX-1)/2),0));
			bp.setSquare(cheese, new Dimension((int)(1+(BX-1)/2),1));
		}
		else{
			bp.setSquare(cheese, new Dimension((int)((BX-1)/2),BY-2));
			bp.setSquare(cheese, new Dimension((int)((BX-1)/2),BY-1));
			bp.setSquare(cheese, new Dimension((int)(1+(BX-1)/2),BY-2));
			bp.setSquare(cheese, new Dimension((int)(1+(BX-1)/2),BY-1));
	...
	void doTraining() {
		...
		System.out.println(reward1.getText()+""+reward2.getText());
		int cval1 = Integer.parseInt(reward1.getText());
		int cval2 = Integer.parseInt(reward2.getText());
		...
		// start training
		trainWorld.agent1Score = cval1;
		trainWorld.agent2Score = cval2;
	...
	void updateScore() {
		double newScore1 = (double)agent1score;
		double newScore2 = (double)agent2score;
		winPerc1.setText(Double.toString(newScore1)+"%");
		winPerc2.setText(Double.toString(newScore2)+"%");
	...
	Container makeParamPanel() {
		...
		labelpane.add(new JLabel("Reward1:", JLabel.RIGHT));
		labelpane.add(new JLabel("Reward2:", JLabel.RIGHT));
		...
		reward1 = new JTextField(20);
		reward2 = new JTextField(20);
		alpha = new JTextField(20);
		gamma = new JTextField(20);
		epsilon = new JTextField(20);
		controlspane.add(reward1);
		controlspane.add(reward2);
	...
	Container makeSettingPanel() {
		...
		clearPolicy.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				game.setPolicy1(rlc.resetLearner1());
				game.setPolicy2(rlc.getPolicy2());
			}
		});
	...
	Container scorePanel() {
		...
		agent1scorelabel = new JLabel(MS_TEXT, mouse, JLabel.RIGHT);
		agent2scorelabel = new JLabel(MS_TEXT, mouse, JLabel.RIGHT);
		...
		winPerc1 = new JLabel("", JLabel.RIGHT); // winning percentage label
		winPerc2 = new JLabel("", JLabel.RIGHT); // winning percentage label
		//hbox.add(reset);
		//hbox.add(winPerc);
		
		//scorePane.add(hbox);

		scorePane.add(agent1scorelabel);
		scorePane.add(agent2scorelabel);
		scorePane.add(winPerc1);
		scorePane.add(winPerc2);
...
class chartPanel extends JPanel {
	...
	int MAXSIZE;
	int lastm1=0, lastc1=0,lastm2=0, lastc2=0;
	
	
	public chartPanel(TwoAgentsApplet a) {
		this.a = a;
		history = new Vector();
	}
	
	public void updateScores() {
		int m1 = a.agent1score;
		int m2 = a.agent2score;
		int dm1 = m1-lastm1;
		int dm2 = m2-lastm2;
		lastm1=m1; 
		lastm2=m2; 
