...
public class TwoAgentsWorld implements RLWorld {
	...
	public int agent2Score = 0;
	public int succesReward1 = 50;
	public int succesReward2 = 400;

	static final int NUM_OBJECTS1 = 3,NUM_OBJECTS2 = 4, NUM_ACTIONS = 8, WALL_TRIALS = 100;
	...
	public int[] getNextState(int action1, int action2) {
		...
		waitingReward2 = calcReward2();

		return getState1();
	}

	private boolean agentsInScoreArea() {
		if (position == 0 && my < 2 && mx >= (int) ((bx - 1) / 2) && mx <= (int) (1 + (bx - 1) / 2) && ny < 2
				&& nx >= (int) ((bx - 1) / 2) && nx <= (int) (1 + (bx - 1) / 2))
			return true;
		else if (position == 1 && my >= by - 2 && mx >= (int) ((bx - 1) / 2) && mx <= (int) (1 + (bx - 1) / 2)
				&& ny >= by - 2 && nx >= (int) ((bx - 1) / 2) && nx <= (int) (1 + (bx - 1) / 2))
			return true;
		else
			return false;
	}
	private boolean agent1InScoreArea() {
		if (position == 0 && my < 2 && mx >= (int) ((bx - 1) / 2) && mx <= (int) (1 + (bx - 1) / 2) )
			return true;
		else if (position == 1 && my >= by - 2 && mx >= (int) ((bx - 1) / 2) && mx <= (int) (1 + (bx - 1) / 2))
			return true;
		else
	...
	public double calcReward1() {
		double newReward = 0;
		if (agent1InScoreArea()) {
			agent1Score++;
			newReward += succesReward1;
            
		}
		System.out.println("reward1 "+newReward+"agent1score"+agent1Score);
		return newReward;
	}
	public double calcReward2() {
		double newReward = 0;
		if (agentsInScoreArea()) {
			agent2Score++;
			newReward += succesReward2;

		}
		System.out.println("reward2 "+newReward+"agent2score"+agent2Score);
		return newReward;
	}
