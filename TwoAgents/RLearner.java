...
public class RLearner {
		...
		case Q_LEARNING : {
			...
			while( ! thisWorld.endState() ) {
			    ...
			    if( ! running ) break;
					...
					action2 = selectAction( state2,2 );
					newstate1 = thisWorld.getNextState( action1,action2 );
				    ...
				    this_Q2 = policy2.getQValue( state2, action2 );
				    ...
				    policy2.setQValue( state2, action2, new_Q2 );
	...
	case SARSA : {
		...
		while( ! thisWorld.endState() ) {
		    ...
		    newaction1 = selectAction( newstate1,1 );
   		    newaction2 = selectAction( newstate2,2 );
		    
   		    this_Q1 = policy1.getQValue( state1, action1 );
   		    next_Q1 = policy1.getQValue( newstate1, newaction1 );
		    this_Q2 = policy2.getQValue( state2, action2 );
		    next_Q2 = policy2.getQValue( newstate2, newaction2 );
		    
		    new_Q1 = this_Q1 + alpha * ( reward1 + gamma * next_Q1 - this_Q1 );
		    new_Q2= this_Q2 + alpha * ( reward2 + gamma * next_Q2 - this_Q2 );
    ...
    private int selectAction( int[] state,int agent ) {
    	double[] qValues;
    	if(agent==1)qValues	= policy1.getQValuesAt( state );
    	else qValues= policy2.getQValuesAt( state );
	...
	case SOFTMAX : {
		...
		if( thisWorld.validAction( selectedAction,agent ) )
		    valid = true;
	    }
	    break;
